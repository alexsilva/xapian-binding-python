...
pkglibexecdir = $(libexecdir)/xapian-bindings
...
DIST_COMMON = $(dist_docdata_DATA) $(dist_exampledata_DATA) \
	$(srcdir)/../generic/generic.mk $(srcdir)/Makefile.am \
	$(srcdir)/Makefile.in $(srcdir)/version.i.in
...
#am__append_2 = modern/xapian_wrap.d modern/xapian_wrap.stamp
subdir = python
...
am__installdirs = "$(DESTDIR)$(pkgpylibdir)" "$(DESTDIR)$(docdatadir)" \
	"$(DESTDIR)$(exampledatadir)" "$(DESTDIR)$(pkgpylibdir)"
LTLIBRARIES = $(pkgpylib_LTLIBRARIES)
am__DEPENDENCIES_1 =
...
DATA = $(dist_docdata_DATA) $(dist_exampledata_DATA) $(pkgpylib_DATA)
ETAGS = etags
CTAGS = ctags
am__tty_colors = \
red=; grn=; lgn=; blu=; std=
DISTFILES = $(DIST_COMMON) $(DIST_SOURCES) $(TEXINFOS) $(EXTRA_DIST)
ACLOCAL = ${SHELL} /xapianb/missing --run aclocal-1.11
AMTAR = $${TAR-tar}
AR = ar
AUTOCONF = ${SHELL} /xapianb/missing --run autoconf
AUTOHEADER = ${SHELL} /xapianb/missing --run autoheader
AUTOM4TE = autom4te
AUTOMAKE = ${SHELL} /xapianb/missing --run automake-1.11
AWK = gawk
BINDINGS =  perl
CC = gcc
CCDEPMODE = depmode=gcc3
CFLAGS = -g -O2
COMPAT_VERSION = 1.2.22
CPP = gcc -E
CPPFLAGS = 
CSC = 
CXX = g++
...
LUA_SO = 
MAINT = #
MAKEINFO = ${SHELL} /xapianb/missing --run makeinfo
MANIFEST_TOOL = :
MKDIR_P = /bin/mkdir -p
MONO = 
NM = /c/MinGW/bin/nm -B
...
PACKAGE_STRING = xapian-bindings 1.2.22
PACKAGE_TARNAME = xapian-bindings
PACKAGE_URL = 
PACKAGE_VERSION = 1.2.22
PATH_SEPARATOR = :
PERL = /bin/perl
PERL_ARCH = /usr/lib/perl5/site_perl/5.6.1/msys
PERL_INC = /usr/lib/perl5/5.6.1/msys/CORE
PERL_LIB = /usr/lib/perl5/site_perl/5.6.1
PERL_SO = .dll
PERL_XAPIAN_VERSION = 1.2.22.0
PHP = 
PHP_CONFIG = 
PHP_EXTENSION_DIR = 
PHP_INC = 
...
TCL_LIBS = 
VERSION = 1.2.22
VERSION_NO_SNAPSHOT = 1.2.22
XAPIAN_CONFIG = /mingw/bin/xapian-config
XAPIAN_CXXFLAGS = -I/mingw/include
XAPIAN_LIBS = -Wl,--enable-runtime-pseudo-reloc /mingw/lib/libxapian.la -lstdc++
XAPIAN_VERSION = 1.2.22
abs_builddir = /xapianb/python
abs_srcdir = /xapianb/python
abs_top_builddir = /xapianb
abs_top_srcdir = /xapianb
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /xapianb/install-sh
...
TESTS_ENVIRONMENT = trap 'rm -f "$(srcdir)"/*.pyc' 0 1 2 13 15; \
    ...
    abs_builddir=/xapianb/python $(PYTHON)
...
CLEANFILES = xapian/_xapian$(PYTHON_SO) xapian/__init__.py \
	xapian/__init__.pyc xapian/__init__.pyo $(srcdir)/*.pyc \
	$(am__append_2)
...
$(ACLOCAL_M4): # $(am__aclocal_m4_deps)
	cd $(top_builddir) && $(MAKE) $(AM_MAKEFLAGS) am--refresh
$(am__aclocal_m4_deps):
version.i: $(top_builddir)/config.status $(srcdir)/version.i.in
	cd $(top_builddir) && $(SHELL) ./config.status $(subdir)/$@
install-pkgpylibLTLIBRARIES: $(pkgpylib_LTLIBRARIES)
	@$(NORMAL_INSTALL)
	@list='$(pkgpylib_LTLIBRARIES)'; test -n "$(pkgpylibdir)" || list=; \
	...
	test -z "$$list2" || { \
	  echo " $(MKDIR_P) '$(DESTDIR)$(pkgpylibdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(pkgpylibdir)" || exit 1; \
	  echo " $(LIBTOOL) $(AM_LIBTOOLFLAGS) $(LIBTOOLFLAGS) --mode=install $(INSTALL) $(INSTALL_STRIP_FLAG) $$list2 '$(DESTDIR)$(pkgpylibdir)'"; \
	  $(LIBTOOL) $(AM_LIBTOOLFLAGS) $(LIBTOOLFLAGS) --mode=install $(INSTALL) $(INSTALL_STRIP_FLAG) $$list2 "$(DESTDIR)$(pkgpylibdir)"; \
	}

uninstall-pkgpylibLTLIBRARIES:
	@$(NORMAL_UNINSTALL)
	@list='$(pkgpylib_LTLIBRARIES)'; test -n "$(pkgpylibdir)" || list=; \
	for p in $$list; do \
	  $(am__strip_dir) \
	  echo " $(LIBTOOL) $(AM_LIBTOOLFLAGS) $(LIBTOOLFLAGS) --mode=uninstall rm -f '$(DESTDIR)$(pkgpylibdir)/$$f'"; \
	  $(LIBTOOL) $(AM_LIBTOOLFLAGS) $(LIBTOOLFLAGS) --mode=uninstall rm -f "$(DESTDIR)$(pkgpylibdir)/$$f"; \
	done

clean-pkgpylibLTLIBRARIES:
	-test -z "$(pkgpylib_LTLIBRARIES)" || rm -f $(pkgpylib_LTLIBRARIES)
	@list='$(pkgpylib_LTLIBRARIES)'; for p in $$list; do \
	  dir="`echo $$p | sed -e 's|/[^/]*$$||'`"; \
	  test "$$dir" != "$$p" || dir=.; \
	  echo "rm -f \"$${dir}/so_locations\""; \
	  rm -f "$${dir}/so_locations"; \
	done
_xapian.la: $(_xapian_la_OBJECTS) $(_xapian_la_DEPENDENCIES) $(EXTRA__xapian_la_DEPENDENCIES) 
	$(_xapian_la_LINK) -rpath $(pkgpylibdir) $(_xapian_la_OBJECTS) $(_xapian_la_LIBADD) $(LIBS)
...
xapian_wrap.lo: modern/xapian_wrap.cc
	...
	$(am__mv) $(DEPDIR)/xapian_wrap.Tpo $(DEPDIR)/xapian_wrap.Plo
...
clean-libtool:
	-rm -rf .libs _libs
install-dist_docdataDATA: $(dist_docdata_DATA)
	@$(NORMAL_INSTALL)
	@list='$(dist_docdata_DATA)'; test -n "$(docdatadir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(docdatadir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(docdatadir)" || exit 1; \
	fi; \
	for p in $$list; do \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(docdatadir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(docdatadir)" || exit $$?; \
	done

uninstall-dist_docdataDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(dist_docdata_DATA)'; test -n "$(docdatadir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(docdatadir)'; $(am__uninstall_files_from_dir)
install-dist_exampledataDATA: $(dist_exampledata_DATA)
	@$(NORMAL_INSTALL)
	@list='$(dist_exampledata_DATA)'; test -n "$(exampledatadir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(exampledatadir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(exampledatadir)" || exit 1; \
	fi; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(exampledatadir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(exampledatadir)" || exit $$?; \
	done
...
uninstall-dist_exampledataDATA:
	...
	@list='$(dist_exampledata_DATA)'; test -n "$(exampledatadir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(exampledatadir)'; $(am__uninstall_files_from_dir)
install-pkgpylibDATA: $(pkgpylib_DATA)
	@$(NORMAL_INSTALL)
	@list='$(pkgpylib_DATA)'; test -n "$(pkgpylibdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(pkgpylibdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(pkgpylibdir)" || exit 1; \
	fi; \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(pkgpylibdir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(pkgpylibdir)" || exit $$?; \
	done

uninstall-pkgpylibDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(pkgpylib_DATA)'; test -n "$(pkgpylibdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(pkgpylibdir)'; $(am__uninstall_files_from_dir)
...
installdirs:
	for dir in "$(DESTDIR)$(pkgpylibdir)" "$(DESTDIR)$(docdatadir)" "$(DESTDIR)$(exampledatadir)" "$(DESTDIR)$(pkgpylibdir)"; do \
...
clean-am: clean-generic clean-libtool clean-local \
	clean-pkgpylibLTLIBRARIES mostlyclean-am
...
uninstall-am: uninstall-dist_docdataDATA \
	...
	uninstall-pkgpylibDATA uninstall-pkgpylibLTLIBRARIES
...
.PHONY: CTAGS GTAGS all all-am check check-TESTS check-am clean \
	...
	clean-pkgpylibLTLIBRARIES ctags distclean distclean-compile \
	...
	install-man install-pdf install-pdf-am install-pkgpylibDATA \
	install-pkgpylibLTLIBRARIES install-ps install-ps-am \
	install-strip installcheck installcheck-am installdirs \
	maintainer-clean maintainer-clean-generic mostlyclean \
	mostlyclean-compile mostlyclean-generic mostlyclean-libtool \
	pdf pdf-am ps ps-am tags uninstall uninstall-am \
	uninstall-dist_docdataDATA uninstall-dist_exampledataDATA \
	uninstall-local uninstall-pkgpylibDATA \
	uninstall-pkgpylibLTLIBRARIES
